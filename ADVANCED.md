...
**Message format:** `getOffer:{username}:{serviceFqn}:{timestamp}`

**Modes:**
1. **Direct lookup** (with @username): Returns specific user's offer
2. **Random** (without @username, no limit): Returns random offer
3. **Paginated** (without @username, with limit): Returns multiple offers

**Example:**
```json
{
  "method": "getOffer",
  "message": "getOffer:bob:chat:1.0.0:1733404800000",
  "signature": "base64-signature",
  "params": {
...
**Message format:** `publishOffer:{username}:{serviceFqn}:{timestamp}`

**Example:**
```json
{
  "method": "publishOffer",
  "message": "publishOffer:alice:chat:1.0.0@alice:1733404800000",
  "signature": "base64-signature",
  "params": {
    "serviceFqn": "chat:1.0.0@alice",
    "offers": [
...
**Message format:** `deleteOffer:{username}:{serviceFqn}:{timestamp}`

**Example:**
```json
{
  "method": "deleteOffer",
  "message": "deleteOffer:alice:chat:1.0.0@alice:1733404800000",
  "signature": "base64-signature",
  "params": {
    "serviceFqn": "chat:1.0.0@alice"
  }
...
- **Message Format**: `publishOffer:{username}:{serviceFqn}:{timestamp}`
- **Auto-Renewal**: Publishing an offer extends username expiry
